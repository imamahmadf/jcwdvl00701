...
function SearchBox(props) {
  ...
  const [propertyNameQuery, setPropertyNameQuery] = useState(
    props.propertyNameQuery
  );

  useEffect(() => {
    setSelectedLocations(
      props.locations
        .filter((loc) => props.selectedLocations.includes(loc.id))
        .map((loc) => {
        ...
        <Flex direction="column">
          ...
          >
            ...
            <Flex
              ...
              mb={2}
              display={isGuestInputOpen ? "flex" : "none"}
            >
              <Box>
          ...
          >
            ...
            <Text
              fontWeight="bold"
              display={!isLocationInputOpen ? "block" : "none"}
            >
            ...
            <Flex
              justify="space-between"
              mb={2}
              display={isLocationInputOpen ? "flex" : "none"}
            >
              <Box
                onClick={(e) => {
                  e.stopPropagation();
          ...
          >
            ...
            <Text
              fontWeight="bold"
              display={!isPropNameInputOpen ? "block" : "none"}
            >
            ...
            <Flex
              justify="space-between"
              mb={2}
              display={isPropNameInputOpen ? "flex" : "none"}
            >
              <Box
                onClick={(e) => {
                  e.stopPropagation();
        ...
        <CardFooter>
          <Stack w="100%">
            ...
            <Text as="b">
              {isDiscount ? (
                <>
                  <Center as={"s"} fontSize={"sm"} color={"gray"}>
                    from Rp.{props.data.defaultPrice},00/per night
                  </Center>
                  <Center color={"red"}>
                    from Rp.{props.data.price},00/per night
                  </Center>
                </>
              ) : (
  ...
  const fetchProperties = useCallback(async () => {
    ...
    response.data.result.properties.forEach((property) => {
      results.push({
        id: property.id,
        pic: property.pic
          ? `${process.env.REACT_APP_API_BASE_URL}${property.pic}`
  ...
  return (
    <div>
      <SearchBox
        ...
        propertyNameQuery={propName}
        setPropertyNameQuery={setPropName}
      />
